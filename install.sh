...
sed -i -e "s@\$NIBBLE_USER_DIR@$CURRENT_USER_DIR@" -e "s@\$NIBBLE_USER@$CURRENT_USER@" -e "s@\$NIBBLE_DIR@$DIR@" /lib/systemd/system/nibble-client.service || { echo "Could not inject directories into client unit file. Exiting... "; exit 1; }
sed -i -e "s@\$NIBBLE_DIR@$DIR@" /lib/systemd/system/nibble-server.service || { echo "Could not inject directories into server unit file. Exiting... "; exit 1; }
...
npm install --prefix $CURRENT_DIR --silent --depth=0
